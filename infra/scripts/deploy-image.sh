...
echo "[1/3] Building image ${IMAGE_TAG} for ${PLATFORM}..."
...
echo "[2/3] Saving and compressing image -> ${ARCHIVE} ..."
docker save "${IMAGE_TAG}" | gzip > "${ARCHIVE}"

if [[ -n "${SKIP_SCP:-}" ]]; then
  echo "[3/3] SKIP_SCP set; skipping transfer."
  echo "Archive ready: ${ARCHIVE}"
  exit 0
fi

echo "[3/3] Transferring ${ARCHIVE} to ${REMOTE_HOST}:${REMOTE_PATH}/"
